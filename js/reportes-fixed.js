...
function render(state) {
    ...
    pageShell("Reportes diarios", "Revisa cada registro por fecha y separa contado, crédito y pagos.", "reportes", `
        ...
        <section class="grid-three report-charts"><article class="panel chart-panel"><div class="panel-header"><h2>Crédito mensual</h2><span class="chart-caption">Córdobas</span></div>${chartBars(monthly, "credit", money, "Crédito")}</article><article class="panel chart-panel"><div class="panel-header"><h2>Pagos mensuales</h2><span class="chart-caption">Córdobas</span></div>${chartBars(monthly, "payments", money, "Pagos")}</article><article class="panel chart-panel"><div class="panel-header"><h2>Inventario del día</h2><span class="chart-caption">Detalle</span></div><div class="inventory-chart"><strong>${data.inventoryCurrent}</strong><span>Stock actual disponible</span><small>Entradas: +${data.inventoryEntries}</small><small>Salidas contado: -${data.inventoryCashOut}</small><small>Salidas crédito: -${data.inventoryCreditOut}</small><small>Movimiento neto: ${data.inventoryMoved >= 0 ? "+" : ""}${data.inventoryMoved}</small><div class="inventory-track"><i style="width:${Math.min(data.inventoryCurrent ? data.inventoryCurrent / Math.max(summary.stock, data.inventoryCurrent) * 100 : 0, 100)}%"></i></div></div></article></section>
